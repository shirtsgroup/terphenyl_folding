...
name='tetramer' # name of molecule. This name will carry through to output
nc=0  # net charge
res='TET' # name of residue being made, NOTE: This must match the residue in the .pdb file or you will get errors
...
if [ "${anneal}" == "yes" ]; then  # anneal if necessary
	gmx grompp -f anneal.mdp -p ${res}.top -c em1.gro -o anneal.tpr
...
python insertmol2charges.py -m ${res}_new.mol2 -t ${res}.top -c ${nc} -o ${res}.top

# Energy minimize again
gmx grompp -f em.mdp -p ${res}.top -c ${res}_new.pdb -o em2
gmx mdrun -nsteps 5000 -pforce 0.1 -v -deffnm em2

...
polymer_length = "pentamer"
polymer_abbreviation = ['P','E','N']
...
# Parameterize our polymer using 'antechamber', from AmberTools.
  ...
  solute_gro_file,gaff_itp_file = parameterize(param_directory,pdb_file,topology_file,polymer_code,polymer_length)
else:
  solute_gro_file,gaff_itp_file = str(str(run_directory)+"/parameterization/"+str(polymer_length)+".gro"),str(str(run_directory)+"/parameterization/"+str(polymer_code)+".top")
...
# Run NPT equilibration of our minimized structure
  equilibration_directory = str(str(run_directory)+"/equilibration")
  equilibrate(equilibration_directory,minimized_pdb_file)
